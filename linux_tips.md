...
如果说 `npm` 是 `apt-get`，那么 **`nvm` 就类似于 Linux 中的 `pyenv` (Python) 或 `rvm` (Ruby)**。它是一个**环境管理器**，专门用来管理 Node.js 本身的不同版本。



| **特性**       | **nvm (版本管理)**                      | **apt-get / Brew (系统管理)**     | **npm (项目依赖管理)**             | **npx (命令执行)**             |
| -------------- | --------------------------------------- | --------------------------------- | ---------------------------------- | ------------------------------ |
| **层级**       | **环境层** (Node 自身版本)              | **系统层** (基础软件)             | **项目层** (代码库/插件)           | **运行层** (临时指令)          |
| **主要任务**   | 切换不同版本的 Node.js                  | 安装各种系统级工具                | 安装和管理项目所需包               | 运行包里的可执行命令           |
| **安装对象**   | 不同版本的 `node` 运行环境              | 全局二进制程序                    | `node_modules` 文件夹里的代码      | 临时或本地的脚本/命令          |
| **生活类比**   | **地基切换器** (决定地基是水泥还是木头) | **大型商场** (买电视、冰箱等大件) | **货架/清单** (为房间买零件和装饰) | **外卖/上门维修** (活干完就走) |
| **解决的问题** | “旧项目要 Node 14，新项目要 Node 18”    | “我电脑上没有 Git/Node”           | “这个项目需要 React 库”            | “我想运行个命令，不想全局安装” |
| **典型命令**   | `nvm install 18`                        | `brew install node`               | `npm install`                      | `npx cap open android`         |